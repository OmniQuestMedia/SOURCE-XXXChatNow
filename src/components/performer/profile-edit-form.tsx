...
import { formatDataHeight, formatDataWeight } from 'src/lib/utils';

import style from './profile-edit-form.module.less';
...
function UserProfile({
  ...
  const [facebook, setFb] = React.useState(socials?.facebook || '');
  const [twitter, setTw] = React.useState(socials?.twitter || '');
  const [instagram, setIta] = React.useState(socials?.instagram || '');
  return (
    <Form
      {...formItemLayout}
      form={form}
      ...
      className={style.performerEditForm}
      initialValues={{
        firstName,
        lastName,
        city,
