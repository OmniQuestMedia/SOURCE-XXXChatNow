...
function LeftHeaderContent({
  ...
  useEffect(() => {
    ...
    else if (pathName.startsWith("/vip-models") || pathName.startsWith("/cams")) setSelectedKey("allModel");
    else if (pathName.startsWith("/live") && !pathName.includes("groupchat")) setSelectedKey("live");
    else if (pathName.includes("groupchat")) setSelectedKey("groupchat");
  }, []);
      ...
      {loggedIn && currentUser && currentUser.isPerformer && currentUser.verified && (
        <>
          ...
          >
            <Link href="/live">
              <a>
                <span className="hidden-lg">Go Live</span>
