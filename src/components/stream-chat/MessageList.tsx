...
class MessageList extends PureComponent<IProps> {
  ...
  componentDidUpdate(prevProps: IProps) {
    ...
    if (prevProps.conversation._id !== conversation._id) {
      this.handleSocketConnect();
    }
  }
    ...
    return (
      ...
      >
        {conversation && conversation._id && (
          <>
            <div className={style['message-list-container']} ref={this.messagesRef}>
              {fetching && <p className={style['text-center']}>fetching...</p>}
              {this.renderMessages()}
            </div>
