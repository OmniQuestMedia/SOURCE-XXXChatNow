...
function PerformerLoginForm({
  ...
  const errorType = () => {
    switch (error.message) {
      ...
      case 'Email is not verified yet':
        return (
          <>
            Please check and verify your email to login. Click
            {' '}
            <Link href="/auth/resend-verification-email">here</Link>
            {' '}
            to resend email verification.
          </>
        );
      default: return getResponseError(error);
    }
  };
  ...
  return (
    <Form layout="vertical" onFinish={onFinish}>
      ...
      >
        <Input onPressEnter={onPressEnter} placeholder="Username or email" />
      </FormItem>
      <FormItem
        hasFeedback
        label={(
...
export default connector(PerformerLoginForm);
