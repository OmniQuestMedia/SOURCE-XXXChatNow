...
class StreamService extends APIRequest {
  getStream(id: string) {
    return this.get(`/streaming/broadcast/${id}`);
  }
  ...
  acceptPrivateChat(id: string) {
    return this.get(`/streaming/private-chat/${encodeURIComponent(id)}`);
  }
  ...
  joinGroupChat(id: string) {
    return this.get(`/streaming/group-chat/${encodeURIComponent(id)}`);
  }
