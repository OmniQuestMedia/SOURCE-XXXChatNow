...
export class PhotoService extends APIRequest {
  ...
  searchAllPhoto(payload: any): Promise<IResponse<IDataResponse<IPhoto>>> {
    return this.get(
  ...
  details(id: string, headers?: { [key: string]: string }) {
    return this.get(`/performer/performer-assets/photos/${encodeURIComponent(id)}/view`, headers);
  }
  ...
  remove(id: string) {
    return this.del(`/performer/performer-assets/photos/${encodeURIComponent(id)}`);
  }
  ...
  create(
    ...
    return new Promise((resolve, reject) => {
      ...
      req.open('POST', isUrl(url) ? url : `${baseApiEndpoint}${url}`);
  ...
  update(
    ...
    return new Promise((resolve, reject) => {
      ...
      req.responseType = 'json';
      req.open('PUT', isUrl(url) ? url : `${baseApiEndpoint}${url}`);
